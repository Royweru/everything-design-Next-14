    ...
    <nav className="flex w-full items-center border-b border-pallete-beige bg-pallete-beige lg:min-h-18 lg:px-[5%]">
      <div className="mx-auto size-full lg:grid lg:grid-cols-[0.375fr_1fr_0.375fr] lg:items-center lg:justify-between lg:gap-4">
        <div className="flex min-h-16 items-center justify-between px-[5%] md:min-h-18 lg:min-h-full lg:px-0">
          <a href={"/"}>
            <img src={'/logo.png'} alt={'Logo'}  height={75} width={75} className=""/>
          </a>
          <div className="flex items-center gap-4 lg:hidden">
            <div>
              {buttons.map((button, index) => (
                <Button key={index} className="w-full px-4 py-1 font-semibold text-pallete-orange" {...button}>
                  {button.title}
                </Button>
              ))}
            </div>
          ...
          {navLinks.map((navLink, index) => (
            <div key={index} className="first:pt-4 lg:first:pt-0">
              {navLink.subMenuLinks && navLink.subMenuLinks.length > 0 ? (
                ...
                <a href={navLink.url} className="block py-3 text-md font-bold lg:px-4 lg:py-2 lg:text-base">
                  {navLink.title}
                </a>
              )}
            </div>
        ...
        <div className="hidden justify-self-end lg:flex items-center gap-x-3">
        <div className=" flex items-center  justify-center">
            <ShoppingCartIcon  className=" size-5 text-text-primary"/>
            </div>
            <div>
            {buttons.map((button, index) => (
            <Button key={index} className="px-6 py-2 font-semibold text-pallete-orange" {...button}>
              {button.title}
            </Button>
          ))}
            </div>
